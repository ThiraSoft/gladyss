...
// TailleEnteteWav est la taille de l'en-tête RIFF/WAVE canonique pour du PCM.
const TailleEnteteWav = 44

// EnveloppeWav préfixe du PCM signé 16 bits little-endian mono d'un en-tête WAV
...
func EnveloppeWav(pcm []byte, sampleRate int) []byte {
	const channels, bits = 1, 16
	const alignment = channels * bits / 8

	out := make([]byte, TailleEnteteWav, TailleEnteteWav+len(pcm))
	copy(out[0:], "RIFF")
...
// TailleInconnueWav est la valeur conventionnelle des champs de taille d'un WAV
// dont on ne connaît pas encore la longueur — celle qu'écrit ffmpeg sur un tube.
const TailleInconnueWav = 0xFFFFFFFF

// EnteteWavStreaming produit l'en-tête d'un WAV dont la longueur est inconnue,
...
func EnteteWavStreaming(sampleRate int) []byte {
	header := EnveloppeWav(nil, sampleRate)
	binary.LittleEndian.PutUint32(header[4:], TailleInconnueWav)
	binary.LittleEndian.PutUint32(header[40:], TailleInconnueWav)
	return header
}
