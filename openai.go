...
type Synthesizer interface {
	...
	SynthesizeTo(ctx context.Context, e synthese.Enonce, out io.Writer) (int, error)
...
func speech(synth Synthesizer, s settings) http.HandlerFunc {
	return func(w http.ResponseWriter, req *http.Request) {
		...
		utterance, err := s.normalize(synthese.Enonce{
			Text:    text,
		...
		if payload.Stream {
			...
			if format == "wav" {
				if _, err := out.Write(synthese.EnteteWavStreaming(sampleRate)); err != nil {
					return
				}
			}
		...
		audio := pcm.Bytes()
		if format == "wav" {
			audio = synthese.EnveloppeWav(audio, sampleRate)
		}
