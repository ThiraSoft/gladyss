...
# où l'outillage de Kyutai dépose les poids. Le téléchargement lui-même vit
# dans synthese (Go, testé) ; ce script ne garde que la confirmation
# interactive et le choix de l'utilisateur.
if [ "$TELECHARGER" -eq 1 ]; then
  etape "Poids du modèle ($LANGUE)"
  if [ "$SANS_QUESTION" -eq 0 ] && [ -t 0 ] && ! poids_presents; then
    printf 'Télécharger les poids depuis %s (CC-BY-4.0) ? [O/n] ' "$DEPOT"
    read -r reponse
    case "$reponse" in [nN]*) TELECHARGER=0 ;; esac
  fi
  if [ "$TELECHARGER" -eq 1 ]; then
    "$RACINE/gladyss" --installer-voix || echec "échec du téléchargement des poids"
  fi
